// app/actions/admin/products/publication/discontinue-product.action.ts
// SERVER ACTION - Arrêt commercial (PUBLISHED -> DISCONTINUED)
"use server";
...
export async function discontinueProductAction(input: unknown) {
  ...
  try {
    await ProductService.discontinue(data.productId, {
      userId: user.id,
      role: user.role,
      reason: data.reason ?? "Arrêt commercial",
    });
