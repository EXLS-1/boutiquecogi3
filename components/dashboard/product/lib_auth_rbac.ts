...
// ───────────────────────────────────────────
// 6. CACHE PROCESS-LEVEL
// ───────────────────────────────────────────

const _permissionCache = new Map<Role, Set<Permission>>();
const _restrictionCache = new Map<Role, Map<Restriction, string | ToggleState>>();
const _configCache = new Map<Role, ReturnType<typeof mergeConfig>>();

function mergeConfig(
  defaults: (typeof DEFAULT_ROLE_CONFIG)[Role],
  dbConfig: { permissions?: Record<string, unknown>; restrictions?: Record<string, unknown> },
) {
  return {
...
async function loadAllRoleConfigs(): Promise<Map<Role, ReturnType<typeof mergeConfig>>> {
  ...
  try {
    ...
    for (const role of Object.values(ROLES)) {
      ...
      _configCache.set(role, dbConfig ? mergeConfig(defaults, dbConfig) : defaults);
    }
  } catch {
    for (const role of Object.values(ROLES)) {
...
export async function getClientRestrictions(
  ...
  const restrictions = await resolveEffectiveRestrictions(role);
  return Object.fromEntries(restrictions) as Record<Restriction, string | ToggleState>;
}
...
export async function getSessionWithUser() {
  ...
  return {
    ...
    role: normalizeRole(
      ((session.user as Record<string, unknown>).role ??
        ((session.user as Record<string, unknown>).metadata as Record<string, unknown>)?.role) as string,
    ),
    level: getRoleLevel(
      normalizeRole(
        ((session.user as Record<string, unknown>).role ??
