...
export const StockService = {
    ...
    async createForProduct(productId: string, initialQty: number = 0, userId?: string) {
    ...
    async reserve(productId: string, quantity: number, orderId: string, userId?: string) {
        return this.adjust(productId, 'RESERVATION', quantity, `Réservation commande ${orderId}`, userId, orderId)
    },

    // ─── Libérer du stock (annulation commande) ───
    async release(productId: string, quantity: number, orderId: string, userId?: string) {
        return this.adjust(productId, 'RELEASE', quantity, `Libération commande ${orderId}`, userId, orderId)
    },
    ...
    computeReserved(type: StockMovementType, qty: number, currentReserved: number): number {
        switch (type) {
            case 'RESERVATION':
