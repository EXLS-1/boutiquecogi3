...
export async function adjustStockAction(formData: FormData): Promise<ActionResult> {
    try {
        ...
        const validTypes: StockMovementType[] = ['IN', 'OUT', 'ADJUSTMENT', 'RETURN']
...
export async function getStockByProductAction(productId: string): Promise<ActionResult> {
    try {
        if (!productId) {
            return {
                success: false,
...
export async function getStockMovementsAction(productId: string): Promise<ActionResult> {
    try {
        if (!productId) {
            return {
                success: false,
...
export async function initStockAction(
    ...
    try {
        ...
        const stock = await StockService.createForProduct(productId, initialQuantity)
        revalidatePath('/admin/products')
        revalidatePath(`/admin/products/${productId}`)
