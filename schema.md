...
model UserAudit {
  ...
  user      User  @relation(fields: [userId], references: [id], onDelete: Cascade)
  createdBy User? @relation("UserAuditCreatedBy", fields: [createdById], references: [id])
  updatedBy User? @relation("UserAuditUpdatedBy", fields: [updatedById], references: [id])
  deletedBy User? @relation("UserAuditDeletedBy", fields: [deletedById], references: [id])
...
model Permission {
  id   String @id @default(uuid(7)) @db.Uuid
  code String @unique // ex: "product:create", "order:refund"
...
model PermissionOverride {
  ...
  // État explicite : ON (true) ou OFF (false)
  isGranted Boolean // true = accordé même si pas dans le rôle, false = retiré même si dans le rôle

  grantedBy String?   @db.Uuid
  grantedAt DateTime  @default(now())
  expiresAt DateTime? // Optionnel : permission temporaire
...
model AuditLog {
  ...
  targetType String? // "ROLE", "USER", "PRODUCT"
  entity     String?
  entityType String?
  entityId   String?
  newValue   Json?
...
model Category {
  ...
  isNavigable Boolean @default(true) // true = visible dans le menu

  managedByRoles     String[] @default([]) // Rôles autorisés à modifier
  requiredPermission String   @default("products:read")
  minRoleLevel       Int      @default(6) // Niveau minimum pour admin
...
model CategoryProduct {
  ...
  categoryId   String @db.Uuid
  displayOrder Int    @default(0) // Permet de trier l'affichage par catégorie
...
model CatalogProduct {
  ...
  productId     String   @db.Uuid
  priceOverride Decimal? @db.Decimal(10, 2) // Surcharge du prix de base pour CE catalogue
  isActive      Boolean  @default(true) // Désactiver un produit dans un catalogue sans le supprimer
...
// Stores actual uploaded media files (images, videos, documents, etc.)
model Media {
  id         String   @id @default(uuid(7)) @db.Uuid
  name       String
  url        String
...
model Coupon {
  ...
  // Promotion directe appliquée aux produits (1 coupon -> N produits)
  products Product[]
...
model WishlistConfig {
  ...
  @@index([feature])
  @@index([requiredPermission, minRoleLevel]) // For efficient permission checks
  @@index([isSharedAllowed]) // For filtering wishlists that can be shared
  @@index([requiresAuth]) // For filtering wishlists that require authentication
  @@index([maxItems]) // For filtering wishlists based on item capacity
  @@map("wishlist_config")
}
...
// ==========================================
// Ce modèle garantit la conservation FOREVER de toutes les données
// liées à une suppression de compte. Aucune suppression physique n'a lieu.
// Les données sont conservées sous forme de snapshot JSON.
// ==========================================
...
model DeletedAccountRegistry {
  ...
  reason   String // Obligatoire — min 5 caractères
  metadata Json? // Infos supplémentaires (ip, userAgent, etc.)
